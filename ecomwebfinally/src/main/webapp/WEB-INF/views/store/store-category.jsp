...
<aside id="sidebar" class="sidebar"
	...
	<ul class="sidebar-nav" id="sidebar-nav">
		<li class="nav-item"><a class="nav-link collapsed"> <i
				class="bi bi-grid"></i> <span>Loại sản phẩm</span></a></li>
	</ul>
	<c:forEach items="${categories}" var="item">
		<ul class="sidebar-nav" id="sidebar-nav">
			<li class="nav-item"><a class="nav-link collapsed"
				href="/store/category/${item.id}"> <i class="bi "></i> <span>${item.name}</span>
			</a></li>
		</ul>
	</c:forEach>
</aside>
					...
					<tbody>
						...
						<c:forEach items="${stores}" var="cate" varStatus="STT">
							<tr>
								...
								<c:url value="/store/${cate.id}" var="URL"></c:url>
								<td><a href="${URL}">${cate.name}</a></td>
								<td>${cate.bio}</td>
								<td>${cate.rating}</td>
								<td>${cate.createat}</td>
							</tr>
